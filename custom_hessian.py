# THIS IS AN ADAPTATION OF https://github.com/amirgholami/PyHessian/blob/master/pyhessian/hessian.py
# PLEASE SEE THE ORIGINAL LICENSING BELOW
...
class hessian():
    ...
    def frobenius(self, maxIter=100):
        """returns estimates of frob norm squares."""
        device = self.device
        squared_estimates = []
        for i in range(maxIter):
            self.model.zero_grad()
            ...
            x = sum([torch.sum(torch.square(s.double())) for s in Hv])
            squared_estimates.append(x.cpu().item())

        return np.array(squared_estimates)
