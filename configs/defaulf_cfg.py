...
class default_parser:
    ...
    def lr_scheduler_parser(self):
        ...
        parser.add_argument('--milestone', type=str, default='10|20|30|40', help="Milestone for MultiStepLRscheduler.")
    ...
    def isam_opt_parser(self):
        parser = argparse.ArgumentParser(add_help=False)
        parser.add_argument('--isam_rho', type=float, default=0.05)
        parser.add_argument('--isam_n_samples', type=int, default=1)
        parser.add_argument('--hessian_n_samples', type=int, default=-1)
        parser.add_argument('--hessian_at_epochs', type=str, default='')
        parser.add_argument('--hessian_every_n_epochs', type=int, default=0)
        parser.add_argument('--hessian_store_full', action='store_true')
        parser.add_argument('--hessian_calc_trace', action='store_true')
        parser.add_argument('--hessian_calc_frobenius', action='store_true')
        parser.add_argument('--hessian_calc_density', action='store_true')
        parser.add_argument('--save_every_epoch', action='store_true')
        parser.add_argument('--hessian_top_n', type=int, default=0)
        parser.add_argument('--save_dir', type=str, default=None)
        parser.add_argument('--isam_setting', type=str, default='trace')
        parser.add_argument('--isam_lam', type=float, default=1.0)
        parser.add_argument('--isam_half_cube_len', type=float, default=1.0)
        parser.add_argument('--isam_sigma', type=float, default=0.5)
        parser.add_argument('--device', type=str, default='cuda:0')
        parser.add_argument('--save_checkpoints_at', type=int, nargs='*', default=[], help="")
        parser.add_argument('--frac_samples', type=float, default=1.0)
        parser.add_argument('--frac_corrupt', type=float, default=0.0)
        # new arg for sam
        parser.add_argument('--sam_variant', type=str, default='sam')
        parser.add_argument('--asam_eta', type=float, default=0.)
    ...
    def get_args(self, args=None):
        ...
        args = final_parser.parse_args(args=args)
    ...
    def auto_set_name(self, args):
        ...
        if args.output_name is None:
            args.output_name = '_'.join([
                ...
                'lr' + str(args.lr),
                str(args.opt),
            ] + sam_hyper_param(args) + ['seed{}'.format(args.seed)] + [str(random.randint(0, 100000))])
